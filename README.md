...
### [BATL v0.10.10(beta) 下载](https://github.com/giga-35b/BA_TimeLiner/releases/download/v0.10.10/BATL_v0.10.10_beta.zip)
<!-- 
### —— 警告 ——
在本项目中你将看到包括但不限于以下内容：
...
~~本工具还未正式发布，欢迎你成为内测用户！若需要反馈bug/提出建议，请直接[联系作者](space.bilibili.com/15097920)~~

**本项目已因屎山堆不下去而放弃，转为考古遗迹。新版请移步[BATL2](https://github.com/giga-35b/BA_TimeLiner_II)**
...
- 在BA玩家摸轴和开票战斗时，为玩家记录各项信息，并**提供直观的可视化情报支持**，如buff、费用等。
- 本工具亦支持作战计划的导入导出，**方便玩家分享和抄作业**。

本工具适用的场景包括但不限于：

- **总力战、大决战、制约解除决战**等战斗的摸轴、分享、抄作业

本工具的功能包括但不限于：

- 摸轴（制定战斗计划）
	- 记录EX、NS、事件的时间点
	- **自动计算并直观显示buff区间与费用情况**
- 实操辅助
	- EX释放参考
	- 实战模拟
- 作业导入/导出
...
本节包含“学生创建/编辑”页面中的各项功能用法<br>
在该页面中，用户可以创建包含基础信息和技能信息的学生数据文件，该文件将用于添加至作战计划

### 学生数据文件

- 学生数据文件为.csv格式，默认保存在./student_files/路径下
- 点击“读取本地学生数据”，即会打开文件对话框，选择打开所需文件即可
- 点击“保存学生数据至本地”，即会将当前页面上所填写的信息保存至默认路径
- “文件信息”组别中，“学生名”为必填项，“保存文件名”可选填，若“保存文件名”空置，则会将学生名作为保存文件名
- 程序会自行添加.csv后缀，用户无需手动添加
- 本工具会在./student_files/路径中附带一部分学生文件示例，可供参考

### 基础信息

- 基础信息为一般不影响作战计划显示的信息（存在例外情况，见下文）
- 包含学生头像、底色、练度信息(星数、等级、技能、装备、爱用品、能力解放、好感)
- 对于“星数”的填写，专武一星-三星分别对应5星-7星
- 对于“装备等级”，随着ba本身的更新，未来的装备等级上限会进一步提高，但本字段不会对后续功能产生任何影响，因此不用过于在意，仅作记录
- 对于“好感”，本字段也不会对后续功能产生任何影响，因此填不填、怎么填都无所谓，仅作记录
- 【注意】对于“技能等级”，部分情况下该值会对作战计划中的费用轴自动计算功能产生影响，详情见下文中的“[设置费用轴](https://github.com/giga-35b/BA_TimeLiner#%E8%AE%BE%E7%BD%AE%E8%B4%B9%E7%94%A8%E8%BD%B4)”部分

### 学生头像及底色

- 学生头像及底色用于作战计划中的该学生相关内容的标示，便于用户区分
- 点击“选择头像图片”将打开文件对话框，选择所需头像即可
- 本工具自带一部分学生头像，保存在./texture\_files/student\_icons/路径下，用户也可以自行添加
- 点击“颜色”左侧的色块可以打开拾色器选取底色，也可以直接点选预设颜色
- 右侧为头像预览。更改底色后，头像预览不会即时更新，需再次选择头像文件后才会更新（预览不影响保存文件中的信息）
- 【注意】学生数据文件中不直接存储头像图片，而是存储和头像图片文件的相对路径，若之后删除或移动头像图片文件，可能导致头像无法正常显示

### 技能信息

- 技能信息用于作战计划中的费用、buff时间等内容的计算
- EX类型/NS类型有默认、伤害、buff三种，在作战计划中应用该技能时，不同技能类型会有不同的视觉效果：
	- 默认：没有效果
	- 伤害：技能目标为敌人时，在“敌人&关卡事件”列中用"hit"标示目标受击时间区间
	- buff：在技能目标对应列中用buff条标示buff持续时间段
- EX默认费用为学生的面板EX技能费用，实际调用时可以对具体实例进行修改
- NS技能CD仅作记录用，不会影响作战计划，作战计划中的NS技能也需要自行设置
- EX前摇/NS前摇为技能释放到实际生效的间隔，即hit标示或buff条会延后该时长，不影响持续时间
- EX持续时间/NS持续时间为技能生效后的持续时间，若技能类型为伤害或buff，则在该时间内会有对应的视觉效果

### 备注

- 备注中可以自由填写任何对于该学生的补充说明，建议在此处说明复杂技能设置方法、使用要点等

## 作战计划

本节包含“作战计划”页面中的各项功能用法<br>
在该页面中，用户可以正式创建包含关卡敌人信息、上场学生、关卡事件、学生技能等信息的作战计划，并生成包含实操参考面板和实操模拟在内的实操辅助信息
...
- 若修改某个学生数据文件，应用了该学生的作战计划文件中的学生信息并不会有变化
- 若需要更新某个作战计划中的学生信息，有两种方法：
	- 将需要更新的学生从作战计划中移除，再添加更新后的学生数据文件。注意：这会导致作战计划中已有的该学生相关技能和buff被清除。
...
- 学生数据文件和作战计划文件都有对应的默认保存/读取文件夹（./student\_files/ 和 ./mission\_files/）
...
- 直接手动修改学生数据文件和关卡计划文件，并改变其格式或结构（并不是不能改，但你需要知道你在改什么，并且修改前请备份）
